  ...
  return (
    <div className="property-page">
      ...
      <section className="shell property-intro">
        ...
        <div className="property-intro__rating"><Star size={17} fill="currentColor" /><strong>{property.rating}</strong><span>{property.reviews} reviews</span></div>
      </section>
      ...
      <div className="shell property-layout">
        <article className="property-main">
          ...
          <section className="reviews-section"><div><h2>Guest and client reviews</h2><p><Star size={17} fill="currentColor" /> {property.rating} from {property.reviews} verified experiences</p></div><div className="review-grid"><article><header><span>TO</span><div><strong>Tomi O.</strong><small>Stayed June 2026</small></div></header><p>Exactly as presented, with thoughtful communication from first message to arrival. The power and Wi-Fi details were accurate.</p></article><article><header><span>NK</span><div><strong>Nneka K.</strong><small>Viewed May 2026</small></div></header><p>A calm, well-organised viewing. I appreciated seeing the full cost breakdown before being asked to make a decision.</p></article></div></section>

          <section className="host-panel"><div className="host-panel__photo"><Image src={property.hostId === "amina-bello" ? "/images/nestora/amina-bello-agent.webp" : property.image} alt={property.host} fill sizes="110px" /></div><div><span className="verified-line verified-line--light"><BadgeCheck size={16} /> Verified professional</span><h2>{property.host}</h2><p>{property.hostId === "amina-bello" ? "Property advisor · Abuja specialist" : "Professional host · Identity checked"}</p><div className="host-panel__actions"><Link href={`/profile/${property.hostId}`} className="button button--outline">View profile</Link><Link href={`/messages?to=${property.hostId}&property=${property.id}`} className="button button--ink"><MessageCircle size={17} /> Message</Link></div></div></section>
