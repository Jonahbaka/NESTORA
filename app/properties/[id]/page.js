    ...
    <div className="property-page">
      ...
      <div className="shell property-layout">
        <article className="property-main">
          <header className="property-summary"><div><p className="eyebrow">{property.tag}</p><h2>{property.title}</h2><p className="property-summary__facts">{property.beds != null ? <span><BedDouble size={18} />{property.beds} bedrooms</span> : null}{property.baths != null ? <span><Bath size={18} />{property.baths} baths</span> : null}{property.areaSqm != null ? <span><Ruler size={18} />{property.areaSqm.toLocaleString("en-NG")} sqm</span> : null}{property.propertyType ? <span><Building2 size={18} />{property.propertyType}</span> : null}</p></div></header>
          ...
          <section className="availability-details"><CalendarCheck2 size={22} /><div><h2>{humanAvailability(property.availabilityStatus)}</h2><p>{property.availableFrom ? `Available from ${new Date(property.availableFrom).toLocaleDateString("en-NG", { day: "numeric", month: "long", year: "numeric", timeZone: "UTC" })}.` : "Contact the listing professional to confirm the current handover date."}</p></div></section>
