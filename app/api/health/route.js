...
export async function GET(request) {
  ...
  if (deep) {
    try {
      ...
      const storageReady = hasRemoteStorage() || process.env.NODE_ENV !== "production" || localRuntime;
      const scannerReady = hasMalwareScanner() || process.env.NODE_ENV !== "production" || localRuntime;
      const sessionSecurityReady = process.env.NODE_ENV !== "production" || String(process.env.NESTORA_SESSION_SECRET || "").length >= 32;
      const deliveryReady = process.env.NODE_ENV !== "production" || localRuntime || Boolean(process.env.NESTORA_DELIVERY_WEBHOOK_URL && process.env.NESTORA_DELIVERY_WEBHOOK_TOKEN && String(process.env.NESTORA_JOB_SECRET || "").length >= 32);
...
function hasSecureProductionOrigin() {
  if (process.env.NODE_ENV !== "production" || isLocalRuntime()) return true;
  try {
