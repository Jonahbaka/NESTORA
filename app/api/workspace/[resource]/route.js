...
const marketingSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("save"), designId: z.uuid().nullable().optional(), name: z.string().trim().min(2).max(160).default("Untitled design"), kind: z.string().trim().min(2).max(80).default("sale_brochure"), canvasWidth: z.number().int().min(200).max(4000).default(595), canvasHeight: z.number().int().min(200).max(4000).default(842), brandKitId: z.uuid().nullable().optional(), elements: z.array(z.any()).max(500).default([]), pages: z.array(z.object({ id: z.string().max(80), name: z.string().max(80), elements: z.array(z.any()).max(500) })).max(40).default([]), documentSettings: z.record(z.any()).default({}), mockupSettings: z.record(z.any()).default({}), dynamicBindings: z.record(z.any()).default({}) }),
  z.object({ action: z.literal("export"), format: z.enum(["pdf", "png", "jpeg"]), designId: z.uuid().nullable().optional(), name: z.string().trim().min(2).max(160).default("Untitled design"), kind: z.string().trim().min(2).max(80).default("sale_brochure"), canvasWidth: z.number().int().min(200).max(4000).default(595), canvasHeight: z.number().int().min(200).max(4000).default(842), brandKitId: z.uuid().nullable().optional(), elements: z.array(z.any()).max(500).default([]), pages: z.array(z.object({ id: z.string().max(80), name: z.string().max(80), elements: z.array(z.any()).max(500) })).max(40).default([]), documentSettings: z.record(z.any()).default({}), mockupSettings: z.record(z.any()).default({}), preset: z.enum(["web", "social", "print", "press"]).default("web"), dpi: z.number().int().min(72).max(600).default(144), exportKind: z.enum(["artwork", "mockup"]).default("artwork"), dynamicBindings: z.record(z.any()).default({}) }),
]);
...
const brandKitSchema = z.object({
  ...
  brandSystem: z.record(z.any()).optional(),
...
const websiteSchema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("create"), name: z.string().trim().min(2).max(120), kind: z.enum(["agent", "agency", "developer", "hospitality", "serviced_apartments", "short_stay"]), templateId: z.string().trim().min(2).max(80), brandKitId: z.uuid().nullable().optional(), sections: z.array(z.string().trim().min(2).max(80)).max(30).default([]), theme: z.record(z.any()).default({}), contact: z.record(z.any()).default({}), seo: z.record(z.any()).default({}), content: z.record(z.any()).default({}), navigation: z.array(z.object({ id: z.string().trim().min(1).max(40), label: z.string().trim().min(1).max(60), visible: z.boolean().default(true) })).max(12).default([]) }),
  z.object({ action: z.literal("update"), websiteId: z.uuid(), name: z.string().trim().min(2).max(120).optional(), templateId: z.string().trim().min(2).max(80).optional(), brandKitId: z.uuid().nullable().optional(), sections: z.array(z.string().trim().min(2).max(80)).max(30).optional(), theme: z.record(z.any()).optional(), contact: z.record(z.any()).optional(), seo: z.record(z.any()).optional(), content: z.record(z.any()).optional(), navigation: z.array(z.object({ id: z.string().trim().min(1).max(40), label: z.string().trim().min(1).max(60), visible: z.boolean() })).max(12).optional() }),
