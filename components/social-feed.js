          ...
          {posts.map((post) => {
            ...
            return (
              <article className="feed-post" key={post.id}>
                <header><Link className="post-avatar" href={post.authorId === "me" ? "/my-nestora" : `/profile/${post.authorId}`}><Image src={post.avatar} alt="" fill sizes="46px" /></Link><div><Link href={post.authorId === "me" ? "/my-nestora" : `/profile/${post.authorId}`}><strong>{post.author}</strong>{post.verified && !post.illustrative ? <BadgeCheck size={15} /> : null}</Link><span>{post.time} · Abuja</span><small className="post-source">{post.sourceLabel || "Community post"}</small></div><button type="button" aria-label="Post options" onClick={() => notify("Post controls opened")}><MoreHorizontal size={19} /></button></header>
                ...
                <div className="post-counts"><span>{post.illustrative ? "Preview engagement" : `${post.reactions + (liked ? 1 : 0)} appreciations`}</span><span>{post.illustrative ? "Sample discussion" : `${post.comments + (comments[post.id]?.length || 0)} comments`}</span></div>
