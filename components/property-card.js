...
export function PropertyCard({ property, priority = false, horizontal = false, headingLevel = 3 }) {
  ...
  const isSaved = mounted && saved.includes(property.id);
  ...
  return (
    <article className={`property-card ${horizontal ? "property-card--horizontal" : ""}`}>
      <div className="property-card__media">
        ...
        <button type="button" className={`save-button ${isSaved ? "active" : ""}`} onClick={() => toggleSaved(property.id)} aria-label={isSaved ? `Remove ${property.title} from saved` : `Save ${property.title}`} aria-pressed={isSaved} disabled={!mounted}><Heart size={19} fill={isSaved ? "currentColor" : "none"} /></button>
      </div>
