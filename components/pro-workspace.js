...
function MediaManager({ listingId, notify, onChanged }) {
  ...
  const [queue, setQueue] = useState([]); const [dragging, setDragging] = useState(false); const fileInputRef = useRef(null);
  ...
  function addFiles(fileList) {
    const next = Array.from(fileList || []).map((file) => ({ id: `${file.name}-${file.size}-${file.lastModified}-${crypto.randomUUID()}`, file, preview: file.type.startsWith("image/") ? URL.createObjectURL(file) : "", status: "ready", progress: 0, error: "" }));
  ...
  async function uploadOne(item, sceneLabel) {
    setQueue((current) => current.map((queued) => queued.id === item.id ? { ...queued, status: "uploading", progress: 18, error: "" } : queued));
    const form = new FormData();
    form.set("listingId", listingId); form.set("mediaRole", uploadRole); form.set("sceneLabel", sceneLabel || ""); form.set("file", item.file);
    setQueue((current) => current.map((queued) => queued.id === item.id ? { ...queued, progress: 62 } : queued));
    const response = await fetch("/api/media", { method: "POST", body: form });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      setQueue((current) => current.map((queued) => queued.id === item.id ? { ...queued, status: "error", progress: 100, error: payload.error || "Upload failed." } : queued));
      return false;
    }
    setQueue((current) => current.map((queued) => queued.id === item.id ? { ...queued, status: "complete", progress: 100 } : queued));
    return true;
  }
  async function upload(event) {
    event.preventDefault();
    const sceneLabel = new FormData(event.currentTarget).get("sceneLabel");
    const pending = queue.filter((item) => item.status === "ready" || item.status === "error");
    if (!pending.length) { notify("Choose one or more files first."); return; }
    let completed = 0;
    for (const item of pending) if (await uploadOne(item, sceneLabel)) completed += 1;
    if (completed) { notify(`${completed} media file${completed === 1 ? "" : "s"} uploaded securely.`); await load(); await onChanged(); }
  }
  ...
  const counts = media.reduce((result, item) => ({ ...result, [normalizedMediaRole(item)]: (result[normalizedMediaRole(item)] || 0) + 1 }), {});
  return <section className="media-manager"><header><div><p className="eyebrow">Listing media command centre</p><h3>Property media</h3><p>Upload several files together, watch each transfer, assign purpose, choose the cover, order the gallery, and label every true 360° room scene.</p></div><Link className="media-service-callout" href={`/services/property-media?listingId=${listingId}`}><Camera size={20} /><span><strong>Need professional production?</strong><small>Book photography, drone, video and hosted 360° capture.</small></span><ChevronRight size={16} /></Link></header><div className="media-summary" aria-label="Media completion summary">{[["cover","Cover"],["gallery","Gallery"],["walkthrough","Video"],["floor_plan","Floor plans"],["panorama","360° scenes"]].map(([key,label]) => <span key={key}><strong>{counts[key] || 0}</strong>{label}</span>)}</div><form className="media-upload-form media-upload-form--premium" onSubmit={upload}><label>Purpose<select name="mediaRole" value={uploadRole} onChange={(event) => setUploadRole(event.target.value)}><option value="gallery">Gallery images</option><option value="cover">Cover image</option><option value="walkthrough">Walkthrough video</option><option value="floor_plan">Floor plans</option><option value="panorama">360° scenes</option></select></label>{uploadRole === "panorama" ? <label>Room label<input name="sceneLabel" minLength={2} maxLength={80} required placeholder="Living room" /></label> : null}<div className={`media-dropzone ${dragging ? "is-dragging" : ""}`} onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); addFiles(event.dataTransfer.files); }}><Upload size={25} /><strong>Drop files here or choose from your device</strong><span>Multiple files supported. Mobile cameras can capture directly.</span><button type="button" onClick={() => fileInputRef.current?.click()}>Choose files</button><input ref={fileInputRef} type="file" accept={mediaAccept(uploadRole)} multiple capture={uploadRole === "gallery" || uploadRole === "cover" ? "environment" : undefined} onChange={(event) => addFiles(event.target.files)} /></div>{queue.length ? <div className="media-upload-queue form-wide">{queue.map((item) => <article key={item.id}>{item.preview ? <Image src={item.preview} alt="" width={46} height={38} unoptimized /> : <span>{item.file.type.startsWith("video/") ? <Video size={20} /> : <FileText size={20} />}</span>}<div><strong>{item.file.name}</strong><small>{formatBytes(item.file.size)} · {humanize(item.status)}</small><progress max="100" value={item.progress} /><em>{item.error}</em></div>{item.status === "error" ? <button type="submit" aria-label={`Retry ${item.file.name}`}><RefreshCw size={15} /></button> : null}<button type="button" onClick={() => discardQueued(item.id)} aria-label={`Remove ${item.file.name} before upload`}><X size={16} /></button></article>)}</div> : null}<div className="form-actions form-wide"><button className="button button--coral" type="submit" disabled={!queue.some((item) => item.status === "ready" || item.status === "error")}><Upload size={16} />Upload ready files</button></div></form>{loading ? <p>Loading media...</p> : <div className="media-list">{media.map((item, index) => <article key={item.id}>{item.kind === "image" ? <a className="media-thumb" href={item.url} target="_blank" rel="noreferrer"><Image src={item.url} alt="" width={54} height={46} unoptimized /></a> : <span>{mediaIcon(item)}</span>}<div className="media-file"><strong>{item.filename}</strong><small>{humanize(item.media_role)} | {formatBytes(item.byte_size)}{item.scene_label ? ` | ${item.scene_label}` : ""} · Ready</small></div><form className="media-metadata" onSubmit={(event) => updateRole(item, event)}><select name="mediaRole" defaultValue={normalizedMediaRole(item)} aria-label={`Purpose for ${item.filename}`}>{mediaRoleOptions(item.kind).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><input name="sceneLabel" defaultValue={item.scene_label || ""} placeholder="Room label for 360" aria-label={`Scene label for ${item.filename}`} /><button type="submit">Save</button></form><div className="media-row-actions"><button type="button" disabled={index === 0} onClick={() => patchMedia({ action: "move", id: item.id, direction: "up" })} aria-label={`Move ${item.filename} up`} title="Move up"><ArrowUp size={15} /></button><button type="button" disabled={index === media.length - 1} onClick={() => patchMedia({ action: "move", id: item.id, direction: "down" })} aria-label={`Move ${item.filename} down`} title="Move down"><ArrowDown size={15} /></button><a href={item.url} target="_blank" rel="noreferrer" aria-label={`Preview ${item.filename}`} title="Preview"><Eye size={15} /></a><button className="media-delete" type="button" onClick={() => remove(item.id)} aria-label={`Remove ${item.filename}`} title="Remove"><Trash2 size={15} /></button></div></article>)}{!media.length ? <p>No media uploaded. A cover image and polished gallery should be your next step.</p> : null}</div>}</section>;
}
