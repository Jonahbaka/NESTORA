...
export async function requireActiveSessionUser(session, runQuery = query) {
  ...
  const result = await runQuery("SELECT id, name, email, role, status, is_demo FROM users WHERE id = $1 LIMIT 1", [session.sub]);
...
export function accessErrorResponse(error) {
  ...
  if (error.code === "CONFLICT") return { status: 409, message: error.message };
  if (error.code === "VALIDATION_ERROR") return { status: 400, message: error.message };
  if (error.code === "SERVICE_UNAVAILABLE") return { status: 503, message: error.message };
  return null;
}
