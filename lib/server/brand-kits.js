...
export async function createBrandKit(input, context) {
  ...
  const result = await query(
    `INSERT INTO brand_kits (external_key, owner_user_id, organization_id, name, is_organization_kit,
      brand_colors, fonts, contact_footer, website_url, social_handles, disclaimer, default_qr_style, approved_images, brand_system)
     VALUES ($1, $2, $3, $4, $5, $6::jsonb, $7::jsonb, $8, $9, $10::jsonb, $11, $12, $13::jsonb, $14::jsonb)
     RETURNING *`,
    [
      ...
      input.disclaimer || null, input.defaultQrStyle || "standard", JSON.stringify(input.approvedImages || []),
      JSON.stringify(input.brandSystem || {}),
    ],
  );
...
export async function updateBrandKit(kitId, input, context) {
  ...
  const normalized = normalizeBrandKitInput(input);
  const allowedFields = ["name", "logo_media_id", "alternate_logo_media_id", "contact_footer", "website_url", "disclaimer", "default_qr_style"];
  const jsonFields = ["brand_colors", "fonts", "social_handles", "approved_images", "brand_system"];
...
export async function lockBrandKit(kitId, context) {
  ...
  await recordAuditEvent({ actorId: context.user.id, action: "brand_kit.locked", targetType: "brand_kit", targetId: kitId });
  return { brandKit: result.rows[0] };
}

export async function unlockBrandKit(kitId, context) {
  if (!context.isAdmin) throw new AccessError("FORBIDDEN", "Administrative access is required.");
  const result = await query("UPDATE brand_kits SET is_locked = false, updated_at = NOW() WHERE id = $1 RETURNING *", [kitId]);
  await recordAuditEvent({ actorId: context.user.id, action: "brand_kit.unlocked", targetType: "brand_kit", targetId: kitId });
  return { brandKit: result.rows[0] };
}
...
function normalizeBrandKitInput(input) {
  return {
    ...
    alternate_logo_media_id: input.alternateLogoMediaId,
    brand_colors: input.brandColors,
    fonts: input.fonts,
    contact_footer: input.contactFooter,
    website_url: input.websiteUrl,
    social_handles: input.socialHandles,
    disclaimer: input.disclaimer,
    default_qr_style: input.defaultQrStyle,
    approved_images: input.approvedImages,
    brand_system: input.brandSystem,
  };
}
