...
export async function findUserByEmail(email) {
  ...
  if (hasDatabase()) {
    const result = await query("SELECT id, name, email, password_hash, role, status FROM users WHERE email = $1 LIMIT 1", [normalized]);
...
export async function findUserById(id) {
  if (hasDatabase()) {
    const result = await query("SELECT id, name, email, role, status, created_at FROM users WHERE id = $1 LIMIT 1", [id]);
    return result.rows[0] || null;
  }
  if (process.env.NODE_ENV === "production") throw new Error("Database is required for account access");
