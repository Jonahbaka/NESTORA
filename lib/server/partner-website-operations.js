...
import { requireFeatureAccess } from "@/lib/server/subscriptions";
...
export async function readWebsiteWorkspace(context) {
  ...
  const [listResult, analyticsResult] = await Promise.all([
    query(
      `SELECT w.id, w.external_key, w.organization_id, w.owner_user_id, w.kind, w.name, w.subdomain, w.status, w.template_id, w.brand_kit_id, w.sections, w.theme, w.contact, w.seo, w.content, w.navigation, w.published_at, w.suspended_at, w.created_at, w.updated_at,
...
export async function readWebsiteEditor(context, websiteId) {
  const website = await getPartnerWebsite(context, websiteId);
...
export async function writeWebsiteEditor(input, context) {
  ...
  if (input.action === "create") {
    const { website } = await createPartnerWebsite(input, context);
  ...
  if (input.action === "update") {
    const { website } = await updatePartnerWebsite(input.websiteId, input, context);
  ...
  if (input.action === "publish") {
    await requireFeatureAccess(context, "partner_website");
    const { website } = await publishPartnerWebsite(input.websiteId, context);
  ...
  if (input.action === "unpublish") {
    const { website } = await unpublishPartnerWebsite(input.websiteId, context);
...
function websitePresentation(website) {
  ...
  const analytics = website.analytics || { visitCount: website.visit_count, visitsLast30Days: website.visits_last_30_days };
  return {
    id: website.id,
    ...
    configuration: {
      ...
      brandKitId: website.brand_kit_id || null,
      brand: { ...(website.theme || {}), ...(website.contact || {}) },
    },
...
async function analyticsResultFor(userId, organizationId) {
  const [visits, enquiries] = await Promise.all([
    ...
    query("SELECT COUNT(*)::int AS count FROM website_enquiries WHERE website_id IN (SELECT id FROM partner_websites WHERE owner_user_id = $1 OR organization_id = $2)", [userId, organizationId]),
  ]);
