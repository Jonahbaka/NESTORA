...
export async function readWebsiteWorkspace(context) {
  ...
  const [listResult, analyticsResult] = await Promise.all([
    query(
      `SELECT w.id, w.external_key, w.organization_id, w.owner_user_id, w.kind, w.name, w.subdomain, w.status, w.template_id, w.brand_kit_id, w.sections, w.theme, w.contact, w.seo, w.published_at, w.suspended_at, w.created_at, w.updated_at,
