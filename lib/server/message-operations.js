  ...
  const result = await query(
    `SELECT c.id, c.subject_type, c.subject_id, c.updated_at, subject_listing.title AS subject_title, subject_listing.location AS subject_location,
     ...
     JOIN LATERAL (
       ...
       WHERE cp.conversation_id = c.id AND cp.user_id <> $1 ORDER BY cp.created_at NULLS LAST LIMIT 1
     ) other_user ON TRUE
     LEFT JOIN LATERAL (
...
export async function startListingConversation({ user, listingId, body, clientNonce }) {
  ...
  try {
    ...
    const listingResult = await runQuery("SELECT id, title, category, owner_user_id, organization_id, status FROM listings WHERE id = $1 AND status = 'active' AND verification_status = 'verified' AND is_demo = FALSE LIMIT 1", [listingId]);
    ...
    const conversationResult = await runQuery(
      `INSERT INTO conversations (external_key, subject_type, subject_id, organization_id)
       VALUES ($1, 'listing', $2, $3)
       ON CONFLICT (external_key) DO UPDATE SET updated_at = NOW()
       RETURNING id`,
      [externalKey, listing.id, listing.organization_id],
    );
    ...
    await runQuery(
      `INSERT INTO leads (external_key, customer_id, owner_user_id, organization_id, listing_id, source, stage, enquiry_text)
       VALUES ($1, $2, $3, $4, $5, 'listing', 'new', $6)
       ON CONFLICT (external_key) DO UPDATE SET enquiry_text = COALESCE(leads.enquiry_text, EXCLUDED.enquiry_text), updated_at = NOW()`,
      [`lead-${digest}`, user.id, routedOwnerId, listing.organization_id, listing.id, body],
    );
