...
export async function createPartnerWebsite(input, context) {
  ...
  const result = await query(
    `INSERT INTO partner_websites (external_key, owner_user_id, organization_id, template_id, name, slug, subdomain, sections, theme, contact, seo)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8::jsonb, $9::jsonb, $10::jsonb, $11::jsonb)
     RETURNING *`,
    [
      externalKey, context.user.id, context.organization?.id || null,
      input.templateId || "default", input.name, slug, slug,
...
export async function getPartnerWebsiteBySubdomain(subdomain) {
  const result = await query(
    "SELECT * FROM partner_websites WHERE subdomain = $1 AND status = 'published' LIMIT 1",
    [subdomain],
  );
