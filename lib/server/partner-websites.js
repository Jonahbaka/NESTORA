...
export async function createPartnerWebsite(input, context) {
  ...
  const result = await query(
    `INSERT INTO partner_websites (external_key, owner_user_id, organization_id, kind, template_id, brand_kit_id, name, slug, subdomain, sections, theme, contact, seo, content, navigation)
...
export async function updatePartnerWebsite(websiteId, input, context) {
  ...
  for (const [key, value] of Object.entries(normalized)) {
    ...
    if (!["name", "sections", "theme", "contact", "seo", "content", "navigation", "template_id", "logo_media_id", "cover_media_id", "brand_kit_id", "analytics_enabled"].includes(key)) continue;
    if (["sections", "theme", "contact", "seo", "content", "navigation"].includes(key)) {
