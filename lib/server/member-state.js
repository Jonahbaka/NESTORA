...
export async function createBookingRequest({ userId, propertyId, roomTypeId, date, guests, nights }) {
  const client = await getPool().connect();
  try {
    await client.query("BEGIN");
    const listingResult = await client.query("SELECT organization_id, title, fees, is_demo FROM listings WHERE id = $1 AND category = 'stay' AND status = 'active' AND verification_status = 'verified' AND (is_demo = FALSE OR $2::boolean) LIMIT 1", [propertyId, allowDemoContent()]);
    ...
    const roomResult = await client.query(
      `SELECT r.id, t.nightly_rate FROM hotel_rooms r JOIN hotel_room_types t ON t.id = r.room_type_id
       WHERE r.organization_id = $1 AND t.listing_id = $2 AND t.id = $3 AND r.status = 'available' AND t.capacity >= $4
         AND NOT EXISTS (SELECT 1 FROM reservations x WHERE x.room_id = r.id AND x.status IN ('requested', 'confirmed') AND daterange(x.check_in, x.check_out, '[)') && daterange($5::date, $6::date, '[)'))
       ORDER BY t.nightly_rate, r.code
       FOR UPDATE OF r SKIP LOCKED
       LIMIT 1`,
      [listing.organization_id, propertyId, roomTypeId, guests, date, checkout],
    );
    if (!roomResult.rowCount) throw operationError("NO_AVAILABILITY", "No room is available for those dates and guest count.");
    const total = Number(roomResult.rows[0].nightly_rate) * nights + publishedFeeTotal(listing.fees);
    ...
    await client.query(
      ...
      [`member-booking-${row.id}`, userId, listing.organization_id, roomResult.rows[0].id, date, checkout, guests, Math.round(total), listing.is_demo],
    );
...
export async function createInspectionRequest({ userId, propertyId, date }) {
  ...
  try {
    await client.query("BEGIN");
    const listingResult = await client.query("SELECT owner_user_id, organization_id, category, title, is_demo FROM listings WHERE id = $1 AND category <> 'stay' AND status = 'active' AND verification_status = 'verified' AND (is_demo = FALSE OR $2::boolean) LIMIT 1", [propertyId, allowDemoContent()]);
    const listing = listingResult.rows[0];
    ...
    const leadResult = await client.query(
      `INSERT INTO leads (external_key, customer_id, owner_user_id, organization_id, listing_id, source, stage, next_action, enquiry_text, is_demo)
       VALUES ($1, $2, $3, $4, $5, 'listing', 'inspection', 'Confirm requested inspection time', 'Inspection requested from the listing page.', $6)
       ON CONFLICT (external_key) DO UPDATE SET stage = 'inspection', next_action = EXCLUDED.next_action, updated_at = NOW()
       RETURNING id`,
      [`member-inspection-${row.id}`, userId, professionalId, listing.organization_id, propertyId, listing.is_demo],
    );
    await client.query(
      `INSERT INTO inspections (external_key, lead_id, customer_id, professional_id, listing_id, scheduled_at, status, is_demo)
       VALUES ($1, $2, $3, $4, $5, ($6::date + TIME '10:00'), 'proposed', $7)`,
      [`member-inspection-${row.id}`, leadResult.rows[0].id, userId, professionalId, propertyId, date, listing.is_demo],
    );
    await client.query(
      `INSERT INTO notifications (user_id, kind, title, body, deep_link, delivery_status)
...
function publishedFeeTotal(value) {
