...
import { getActiveSubscription, getAllPlanDefinitions, requireFeatureAccess } from "@/lib/server/subscriptions";
import { createPartnerWebsite, getPartnerWebsite, listPartnerWebsites, updatePartnerWebsite, publishPartnerWebsite, unpublishPartnerWebsite, suspendPartnerWebsite, reinstatePartnerWebsite, getWebsiteAnalytics } from "@/lib/server/partner-websites";
import { createBrandKit, getBrandKit, listBrandKits, updateBrandKit, deleteBrandKit, lockBrandKit, unlockBrandKit } from "@/lib/server/brand-kits";
import { createDesign, getDesign, listDesigns, listTemplates, updateDesign, duplicateDesign, archiveDesign, saveAsTemplate, approveTemplate, exportDesign } from "@/lib/server/marketing-studio";
import { readWebsiteWorkspace, readWebsiteEditor, writeWebsiteEditor, writeWebsiteAdministration, listWebsiteTemplates } from "./partner-website-operations";
import { getAnalytics, getListingAnalytics, exportAnalyticsCsv } from "@/lib/server/analytics";

const professionalResources = new Set(["overview", "profile", "listings", "leads", "inspections", "entitlements", "marketing", "subscription", "websites", "brand-kits", "templates"]);
...
export async function readWorkspaceResource(resource, context) {
  ...
  if (resource === "subscription") return readSubscription(context);
  if (resource === "websites") return readWebsiteWorkspace(context);
  if (resource === "brand-kits") return readBrandKits(context);
  if (resource === "templates") return readTemplates(context);
...
export async function writeWorkspaceResource(resource, input, context) {
  ...
  if (resource === "subscription") return writeSubscription(input, context);
  if (resource === "websites") return writeWebsiteEditor(input, context);
  if (resource === "brand-kits") {
    if (input.action === "delete") return deleteBrandKit(input.brandKitId, context);
    const result = input.action === "create" ? await createBrandKit(input, context)
      : input.action === "update" ? await updateBrandKit(input.brandKitId, input, context)
      : input.action === "lock" ? await lockBrandKit(input.brandKitId, context)
      : input.action === "unlock" ? await unlockBrandKit(input.brandKitId, context)
      : null;
    if (result?.brandKit) return { brandKit: serializeBrandKit(result.brandKit) };
    throw new AccessError("NOT_FOUND", "Brand kit action not found.");
  }
  if (resource === "templates") {
    if (input.action === "create") {
      if (!input.name || !input.kind) throw new AccessError("VALIDATION_ERROR", "Template name and kind are required.");
      return createDesign({ ...input, canvasPreset: input.canvasPreset || "a4" }, context);
    }
    if (input.action === "duplicate") return duplicateDesign(input.designId, context);
    throw new AccessError("NOT_FOUND", "Template action not found.");
  }
  throw new AccessError("NOT_FOUND", "Workspace resource not found.");
}
...
async function readAdministration(context) {
  const [reports, verifications, conversationReports, pendingListings, users, organizations, subscriptions, audit, incidents, plans, websites] = await Promise.all([
    ...
    query("SELECT plan_id, name, monthly_price_ngn, annual_price_ngn, limits, features, is_active FROM plan_definitions ORDER BY monthly_price_ngn ASC"),
    query(`SELECT w.id, w.name, w.subdomain, w.status, w.published_at, w.owner_user_id, u.name AS owner_name, o.name AS organization_name
           FROM partner_websites w LEFT JOIN users u ON u.id = w.owner_user_id LEFT JOIN organizations o ON o.id = w.organization_id
           ORDER BY w.updated_at DESC LIMIT 300`),
  ]);
  return {
    reports: reports.rows,
    conversationReports: conversationReports.rows,
    verifications: verifications.rows,
    pendingListings: pendingListings.rows,
    users: context.isAdmin ? users.rows : [],
    organizations: context.isAdmin ? organizations.rows : [],
    subscriptions: subscriptions.rows.map((row) => ({ ...row, starts_at: row.starts_at, ends_at: row.ends_at })),
    plans: plans.rows,
    websites: websites.rows.map((row) => ({ ...row, visitCount: 0, visitsLast30Days: 0 })),
    auditEvents: audit.rows,
    incidents: incidents.rows,
  };
}

export { readAdministration, writeAdministration };

async function readSubscription(context) {
  const active = await getActiveSubscription(context);
  const plans = await getAllPlanDefinitions();
  return { subscription: serializeSubscription(active), plans };
}

async function writeSubscription(input, context) {
  if (input.action === "requestUpgrade") {
    if (!["agent", "agency_admin", "developer", "host"].includes(context.user.role)) throw new AccessError("FORBIDDEN", "Subscription requests are limited to professional accounts.");
    if (!input.planId) throw new AccessError("VALIDATION_ERROR", "Select a plan for the upgrade request.");
    const existing = await getActiveSubscription(context);
    if (existing && existing.plan_id === input.planId) throw new AccessError("VALIDATION_ERROR", "You are already on this plan.");
    const result = await query(
      `INSERT INTO entitlement_history (subscription_id, actor_id, action, metadata)
       VALUES ($1, $2, 'upgrade_requested', $3)
       RETURNING id, metadata, created_at`,
      [existing?.id || null, context.user.id, JSON.stringify({ requested_plan_id: input.planId, current_plan_id: existing?.plan_id || null, organisation_id: context.organization?.id || null })],
    );
    return { request: result.rows[0] };
  }
  throw new AccessError("NOT_FOUND", "Subscription action not found.");
}
...
async function readEntitlements(context) {
  ...
  const plans = await getAllPlanDefinitions();
  return { subscription, plans, entitlements: result.rows.filter((row) => row.feature_key).map((row) => ({ key: row.feature_key, enabled: row.enabled, limit: row.limit_value })) };
}

async function readBrandKits(context) {
  const kits = await listBrandKits(context);
  const organizationId = context.organization?.id || null;
  const websites = await query(
    `SELECT id, name FROM partner_websites
     WHERE owner_user_id = $1 OR ($2::uuid IS NOT NULL AND organization_id = $2)
     ORDER BY updated_at DESC LIMIT 50`,
    [context.user.id, organizationId],
  );
  return { kits: kits.map(serializeBrandKit), websites: websites.rows };
}

async function readTemplates(context) {
  const templates = await listTemplates(context);
  const designs = await listDesigns(context, { isTemplate: true });
  return { templates, designs };
}

async function readMarketing(context) {
  const scope = scoped(context, { alias: "m" });
  const mediaScope = scoped(context, { alias: "l" });
  const organizationId = context.organization?.id || null;
  const [result, listingData, designs, templates, kits, entitlementData, developments, units, usage, mediaLibrary] = await Promise.all([
    query(
    ...
    listDesigns(context, { limit: 50 }),
    listTemplates(context),
    listBrandKits(context),
    readEntitlements(context),
    query("SELECT id, name, location, status, completion_date, construction_progress, payment_plan_summary FROM developments WHERE $1::uuid IS NOT NULL AND organization_id = $1 ORDER BY updated_at DESC LIMIT 50", [organizationId]),
    query("SELECT u.id, u.code, u.status, u.price_amount, ut.name AS unit_type, d.name AS development_name FROM units u JOIN unit_types ut ON ut.id = u.unit_type_id JOIN developments d ON d.id = u.development_id WHERE $1::uuid IS NOT NULL AND d.organization_id = $1 ORDER BY u.updated_at DESC LIMIT 100", [organizationId]),
    query(`SELECT
      (SELECT COUNT(*)::int FROM marketing_designs WHERE owner_user_id = $1 OR ($2::uuid IS NOT NULL AND organization_id = $2)) AS designs,
      (SELECT COUNT(*)::int FROM design_exports de JOIN marketing_designs md ON md.id = de.design_id WHERE md.owner_user_id = $1 OR ($2::uuid IS NOT NULL AND md.organization_id = $2)) AS exports,
      (SELECT COUNT(*)::int FROM partner_websites WHERE owner_user_id = $1 OR ($2::uuid IS NOT NULL AND organization_id = $2)) AS websites`, [context.user.id, organizationId]),
    query(
      `SELECT lm.id, lm.filename, lm.kind, lm.media_role, lm.media_source, lm.service_booking_id,
              lm.created_at, l.id AS listing_id, l.title AS listing_title
       FROM listing_media lm JOIN listings l ON l.id = lm.listing_id
       WHERE ${mediaScope.clause} AND lm.status = 'ready'
       ORDER BY CASE WHEN lm.media_source <> 'user_upload' THEN 0 ELSE 1 END, lm.created_at DESC
       LIMIT 200`,
      mediaScope.values,
    ),
  ]);
  const latestDraft = designs.find((design) => design.status === "draft") || designs[0] || null;
  return {
    materials: result.rows.map((item) => ({ ...item, previewPath: item.storage_key ? `/api/marketing/${item.id}` : null })),
    listings: listingData.listings.map(({ id, title }) => ({ id, title })),
    designs: designs.map((row) => ({
      ...
      mockupSettings: row.mockup_settings || {},
      dynamicBindings: row.dynamic_bindings,
    ...
    latestDraft: latestDraft
      ? {
          ...
          canvasWidth: latestDraft.canvas_width,
          canvasHeight: latestDraft.canvas_height,
          brandKitId: latestDraft.brand_kit_id,
          elements: latestDraft.elements,
          pages: latestDraft.pages || [],
          documentSettings: latestDraft.document_settings || {},
          mockupSettings: latestDraft.mockup_settings || {},
          dynamicBindings: latestDraft.dynamic_bindings,
        }
      : null,
    templates: templates.map((row) => ({ id: row.id, name: row.name, kind: row.kind, canvasWidth: row.canvas_width, canvasHeight: row.canvas_height, elements: row.elements, dynamicBindings: row.dynamic_bindings, brandKitId: row.brand_kit_id })),
    brandKits: kits.map(serializeBrandKit),
    mediaLibrary: mediaLibrary.rows.map((row) => ({ ...row, url: `/api/media/${row.id}` })),
    developments: developments.rows,
    units: units.rows.map((row) => ({ ...row, price_amount: Number(row.price_amount) })),
    entitlements: entitlementData.entitlements,
    usage: usage.rows[0] || { designs: 0, exports: 0, websites: 0 },
...
async function writeMarketing(input, context) {
  ...
  if (input.action === "save") {
    ...
    if (input.designId) {
      await getDesign(context, input.designId);
      const result = await updateDesign(input.designId, { name: input.name, brand_kit_id: input.brandKitId || null, elements: input.elements, pages: input.pages, document_settings: input.documentSettings, mockup_settings: input.mockupSettings, dynamic_bindings: input.dynamicBindings, canvas_width: input.canvasWidth, canvas_height: input.canvasHeight }, context);
      return { designId: result.design.id, saved: true };
    }
    const result = await createDesign({ ...input, canvasPreset: canvasPresetFor(input.canvasWidth, input.canvasHeight) }, context);
    return { designId: result.design.id, saved: true };
  }
  if (input.action === "export") {
    await requireFeatureAccess(context, input.format === "pdf" ? "pdf_export" : "image_export");
    let designId = input.designId;
    if (!designId) {
      const result = await createDesign({ ...input, canvasPreset: canvasPresetFor(input.canvasWidth, input.canvasHeight) }, context);
      designId = result.design.id;
    }
    await getDesign(context, designId);
    if (input.pages?.length || input.documentSettings || input.mockupSettings) await updateDesign(designId, { elements: input.elements, pages: input.pages, document_settings: input.documentSettings, mockup_settings: input.mockupSettings }, context);
    const exportResult = await exportDesign(designId, input.format, context, { preset: input.preset, dpi: input.dpi, exportKind: input.exportKind });
    await recordAuditEvent({ actorId: context.user.id, action: "marketing_design.exported", targetType: "marketing_design", targetId: designId, metadata: { format: input.format } });
    return { designId, downloadUrl: `/api/design-exports/${exportResult.exportId}` };
  }
...
function serializeBrandKit(row) {
  return {
    id: row.id,
    name: row.name,
    brandColors: row.brand_colors,
    fonts: row.fonts,
    contactFooter: row.contact_footer,
    websiteUrl: row.website_url,
    socialHandles: row.social_handles,
    disclaimer: row.disclaimer,
    defaultQrStyle: row.default_qr_style,
    logoMediaId: row.logo_media_id,
    alternateLogoMediaId: row.alternate_logo_media_id,
    approvedImages: row.approved_images || [],
    brandSystem: row.brand_system || {},
    isLocked: row.is_locked,
    isOrganizationKit: row.is_organization_kit,
  };
}
...
function canvasPresetFor(width, height) {
  if (width === 500 && height === 500) return "square";
  if (width === 400 && height === 600) return "portrait";
  if (width === 360 && height === 640) return "story";
  if (width === 800 && height === 500) return "landscape";
  return "a4";
}
