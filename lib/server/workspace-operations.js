...
import { getActiveSubscription, getAllPlanDefinitions } from "@/lib/server/subscriptions";
...
export async function writeWorkspaceResource(resource, input, context) {
  ...
  if (resource === "brand-kits") {
    if (input.action === "create") return createBrandKit(input, context);
    if (input.action === "update") return updateBrandKit(input.brandKitId, input, context);
    if (input.action === "delete") return deleteBrandKit(input.brandKitId, context);
    if (input.action === "lock") return lockBrandKit(input.brandKitId, context);
    if (input.action === "unlock") return unlockBrandKit(input.brandKitId, context);
    throw new AccessError("NOT_FOUND", "Brand kit action not found.");
  }
  if (resource === "templates") {
    if (input.action === "create") return createDesign(input, context);
...
async function readSubscription(context) {
  const subscription = await getActiveSubscription(context.user.id, context.organization?.id || null);
...
async function writeSubscription(input, context) {
  if (input.action === "requestUpgrade") {
    ...
    const existing = await getActiveSubscription(context.user.id, context.organization?.id || null);
...
async function readMarketing(context) {
  ...
  const [result, listingData, designs, templates, kits] = await Promise.all([
    query(
  ...
  const latestDraft = designs.rows.find((design) => design.status === "draft") || designs.rows[0] || null;
  return {
    ...
    latestDraft: latestDraft
      ...
      : null,
    templates: templates.rows.map((row) => ({ id: row.id, name: row.name, kind: row.kind, canvasWidth: row.canvas_width, canvasHeight: row.canvas_height, elements: row.elements })),
    brandKits: kits.rows.map(serializeBrandKit),
...
async function writeMarketing(input, context) {
  ...
  if (input.action === "save") {
    await requireFeature(context, "marketing_studio");
    if (input.designId) {
      const design = await getDesign(context, input.designId);
      const updates = { elements: input.elements, canvas_width: 595, canvas_height: 420 };
      if (input.name) updates.name = input.name;
      if (input.kind) updates.kind = input.kind;
      if (input.brandKitId !== undefined) updates.brand_kit_id = input.brandKitId;
      const result = await updateDesign(input.designId, updates, context);
      return { designId: result.design.id, saved: true };
    }
    const result = await createDesign({
      name: input.name || "Untitled design",
      kind: input.kind || "rental_flyer",
      canvasPreset: input.canvasPreset || "a4",
      canvasWidth: input.canvasWidth || 595,
      canvasHeight: input.canvasHeight || 420,
      brandKitId: input.brandKitId || null,
      elements: input.elements || [],
      dynamicBindings: input.dynamicBindings || {},
    }, context);
    return { designId: result.design.id, saved: true };
  }
  if (input.action === "export") {
    await requireFeature(context, input.format === "pdf" ? "pdf_export" : "image_export");
    let designId = input.designId;
    if (!designId) {
      const result = await createDesign({ ...input, canvasPreset: "a4", elements: input.elements }, context);
    ...
    // Return an authenticated download URL instead of exposing the private storage key
    const baseUrl = process.env.NEXT_PUBLIC_APP_ORIGIN || (process.env.NODE_ENV === "production" ? "" : "http://localhost:3000");
    return { designId, downloadUrl: `${baseUrl}/api/design-exports/${exportResult.exportId}`, exportId: exportResult.exportId, contentType: exportResult.contentType, fileSize: exportResult.fileSize };
  }
