...
const professionalResources = new Set(["overview", "listings", "leads", "inspections", "entitlements", "marketing"]);
...
function scoped(context, options = {}) {
  if (context.user.role === "agent" && !context.isAdmin) {
    ...
    return { clause: `${alias}${options.owner || "owner_user_id"} = $1`, values: [context.user.id], next: 2 };
  }
...
async function readListings(context) {
  ...
  const result = await query(
    `SELECT l.id, l.title, l.category, l.location, l.price_amount, l.currency, l.status, l.verification_status,
            l.description, l.bedrooms, l.bathrooms, l.updated_at, COUNT(lm.id)::int AS media_count
     FROM listings l
  ...
  return { listings: result.rows.map((row) => ({ ...row, price_amount: Number(row.price_amount), bathrooms: row.bathrooms == null ? null : Number(row.bathrooms) })) };
}
...
async function readLeads(context) {
  ...
  const scope = scoped(context, { alias: "l" });
  const result = await query(
    `SELECT l.id, l.external_key, l.source, l.stage, l.priority, l.next_action, l.enquiry_text, l.created_at, l.updated_at,
            customer.name AS customer_name, customer.email AS customer_email, owner.name AS owner_name,
...
async function readHotel(context) {
  ...
  const [roomTypes, rooms, reservations] = await Promise.all([
    query("SELECT id, code, name, capacity, nightly_rate FROM hotel_room_types WHERE organization_id = $1 ORDER BY name", [organization.id]),
    query(`SELECT r.id, r.code, r.status, rt.name AS room_type, rt.capacity, rt.nightly_rate
...
async function writeListing(input, context) {
  ...
  if (input.action === "create") {
    ...
    const organizationId = context.organization?.id || null;
    const result = await query(
      `INSERT INTO listings (id, owner_user_id, organization_id, title, category, location, price_amount, status, description, bedrooms, bathrooms, verification_status)
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'draft', $8, $9, $10, 'pending')
       RETURNING id, title, category, location, price_amount, status, description, bedrooms, bathrooms, updated_at`,
      [id, context.user.id, organizationId, input.title, input.category, input.location, input.priceAmount, input.description || null, input.bedrooms ?? null, input.bathrooms ?? null],
    );
  ...
  if (input.action === "update") {
    ...
    if (input.status === "active") {
      ...
      const media = await query("SELECT 1 FROM listing_media WHERE listing_id = $1 AND kind = 'image' AND status = 'ready' LIMIT 1", [input.id]);
      if (!media.rowCount) throw new AccessError("FORBIDDEN", "Upload at least one valid property image before publishing.");
    }
    const values = [input.title, input.location, input.priceAmount, input.description || null, input.status, input.id, ...scope.values];
    const result = await query(
      `UPDATE listings SET title = $1, location = $2, price_amount = $3, description = $4, status = $5,
                           published_at = CASE WHEN $5 = 'active' AND published_at IS NULL THEN NOW() ELSE published_at END, updated_at = NOW()
       WHERE id = $6 AND ${shiftPlaceholders(scope.clause, 6)}
       RETURNING id, title, category, location, price_amount, status, description, bedrooms, bathrooms, updated_at`,
      values,
    );
    const listing = requireRow(result, "Listing not found.");
    await recordAuditEvent({ actorId: context.user.id, action: "listing.updated", targetType: "listing", targetId: input.id, metadata: { status: input.status } });
    return { listing };
  }
...
async function writeHotel(input, context) {
  ...
  if (input.action === "roomType") {
    await requireFeature(context, "hotel_inventory");
    const result = await query(
      `INSERT INTO hotel_room_types (organization_id, code, name, capacity, nightly_rate) VALUES ($1, $2, $3, $4, $5)
       RETURNING id, code, name, capacity, nightly_rate`,
      [organization.id, input.code, input.name, input.capacity, input.nightlyRate],
    );
    await recordAuditEvent({ actorId: context.user.id, action: "hotel_room_type.created", targetType: "hotel_room_type", targetId: result.rows[0].id, metadata: { organizationId: organization.id } });
    return { roomType: result.rows[0] };
  }
...
async function writeAdministration(input, context) {
  ...
  if (input.action === "listingDecision") {
    if (input.status === "approved") {
      const media = await query("SELECT 1 FROM listing_media WHERE listing_id = $1 AND kind = 'image' AND status = 'ready' LIMIT 1", [input.id]);
      if (!media.rowCount) throw new AccessError("FORBIDDEN", "A listing must have a valid image before approval.");
    }
    const result = await query(
      `UPDATE listings SET verification_status = $1,
                           ...
                           updated_at = NOW()
       WHERE id = $3 RETURNING id, title, status, verification_status, updated_at`,
      [input.status === "approved" ? "verified" : input.status === "rejected" ? "rejected" : "verified", input.status, input.id],
    );
    const listing = requireRow(result, "Listing not found.");
...
async function writeMarketing(input, context) {
  ...
  const result = await query(
    `INSERT INTO marketing_materials (external_key, owner_user_id, organization_id, listing_id, development_id, kind, status, qr_target)
     VALUES ($1, $2, $3, $4, $5, $6, 'draft', $7) RETURNING id, external_key, kind, status, qr_target, created_at`,
    [externalKey, context.user.id, organizationId, input.listingId || null, input.developmentId || null, input.kind, input.qrTarget || null],
  );
  const material = result.rows[0];
  const qrTarget = absoluteApplicationPath(input.qrTarget || (listing ? `/properties/${listing.id}` : "/workspace"));
...
function absoluteApplicationPath(value) {
  ...
  if (process.env.NODE_ENV === "production" && origin.protocol !== "https:") throw new AccessError("SERVICE_UNAVAILABLE", "The public application origin must use HTTPS.");
