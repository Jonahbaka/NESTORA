...
async function readBrandKits(context) {
  ...
  return { kits: kits.map(serializeBrandKit) };
}
...
async function readMarketing(context) {
  ...
  const organizationId = context.organization?.id || null;
  const [result, listingData, designs, templates, kits, entitlementData, developments, units, usage] = await Promise.all([
    query(
...
async function writeMarketing(input, context) {
  ...
  if (input.action === "save") {
    ...
    if (input.designId) {
      const design = await getDesign(context, input.designId);
      const result = await updateDesign(input.designId, { name: input.name, brand_kit_id: input.brandKitId || null, elements: input.elements, dynamic_bindings: input.dynamicBindings, canvas_width: input.canvasWidth, canvas_height: input.canvasHeight }, context);
  ...
  if (input.action === "export") {
    ...
    const exportResult = await exportDesign(designId, input.format, context);
