...
export async function listTemplates(context, { kind } = {}) {
  ...
  const result = await query(
    `SELECT md.*, bk.name AS brand_kit_name
     ...
     LEFT JOIN brand_kits bk ON bk.id = md.brand_kit_id
     WHERE ${conditions.join(" AND ")}
     ORDER BY md.is_approved_template DESC, md.updated_at DESC LIMIT 100`,
    values,
  );
  return result.rows;
}
...
export async function createDesign(input, context) {
  ...
  const result = await query(
    `INSERT INTO marketing_designs (external_key, owner_user_id, organization_id, brand_kit_id, name, kind, template_id,
      is_template, is_organization_template, canvas_width, canvas_height, elements, dynamic_bindings, pages, document_settings, mockup_settings)
    ...
    [
      ...
      JSON.stringify(input.documentSettings || {}),
      JSON.stringify(input.mockupSettings || {}),
    ],
  );
...
export async function updateDesign(designId, input, context) {
  ...
  const jsonFields = ["elements", "dynamic_bindings", "pages", "document_settings", "mockup_settings"];
...
export async function duplicateDesign(designId, context) {
  ...
  const result = await query(
    `INSERT INTO marketing_designs (external_key, owner_user_id, organization_id, brand_kit_id, name, kind, template_id,
      canvas_width, canvas_height, elements, dynamic_bindings, pages, document_settings, mockup_settings)
    ...
    [
      ...
      JSON.stringify(existing.pages || []), JSON.stringify(existing.document_settings || {}), JSON.stringify(existing.mockup_settings || {}),
    ],
  );
  const design = result.rows[0];
  await recordAuditEvent({
...
export async function exportDesign(designId, format, context, options = {}) {
  ...
  const result = await query(
    `INSERT INTO design_exports (design_id, format, storage_key, file_size_bytes, preset, dpi, export_kind)
     VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
    [designId, format, storageKey, buffer.length, options.preset || "web", options.dpi || 144, options.exportKind || "artwork"],
  );
...
async function renderDesignToPdf(design) {
  ...
  const pages = Array.isArray(design.pages) && design.pages.length ? design.pages : [{ elements: design.elements || [] }];
  ...
  return new Promise((resolve, reject) => {
    ...
    (async () => {
      for (let index = 0; index < pages.length; index += 1) {
        if (index) doc.addPage({ size: [width, height], margin: 0 });
        const pageImage = await renderDesignToImage({ ...design, elements: pages[index].elements || [] }, "png");
